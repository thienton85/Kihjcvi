...
SCRIPTS = {
    "1": {"name": "Script 1: Đăng nhập", "url": "https://raw.githubusercontent.com/Khanh23047/Golike-likedin/refs/heads/main/p.py"},
    "2": {"name": "Script 2: Tìm kiếm", "url": "https://raw.githubusercontent.com/Khanh23047/Golike-likedin/refs/heads/main/p.py"},
...
def display_menu():
    """Hiển thị giao diện chính VIP với SCODE-V1 to và đậm hơn"""
    clear_screen()
    
    # Giai đoạn 1: Header với gradient và SCODE-V1 nổi bật
    colors = [Fore.CYAN, Fore.BLUE, Fore.MAGENTA, Fore.YELLOW]
    header_text = "=> SCODE <= | Phiên bản: V1"  # Thêm ký tự █ để to và đậm
    for i in range(5):
        clear_screen()
        border = f"{colors[i % len(colors)]}╔═{'═' * 58}═╗{Style.RESET_ALL}"
        print(border)
        padding = " " * (i % 3)
        centered_text = f"{padding}{header_text}{padding}".center(60)
        print(f"{colors[i % len(colors)]}║ {Style.RESET_ALL}{Fore.WHITE}{Style.BRIGHT}{centered_text}{Style.RESET_ALL}║")
        print(f"{colors[i % len(colors)]}╚═{'═' * 58}═╝{Style.RESET_ALL}")
        print(f"{Fore.GREEN}✦ Đang khởi tạo giao diện VIP...{Style.RESET_ALL}")
        time.sleep(0.1)
    
    # Giai đoạn 2: Thông tin hệ thống với viền đôi và ô vuông
    clear_screen()
    print(f"{Fore.BLUE}╔═{'═' * 58}═╗{Style.RESET_ALL}")
    gradient_sweep(f"║ {header_text.center(58)} ║", [Fore.CYAN, Fore.BLUE, Fore.MAGENTA], 2)
    print(f"{Fore.BLUE}╚═{'═' * 58}═╝{Style.RESET_ALL}")
    
    time.sleep(0.2)
    print(f"{Fore.GREEN}╔═{'═' * 58}═╗{Style.RESET_ALL}")
    fade_in_line(f"║ {Style.BRIGHT}THÔNG TIN HỆ THỐNG{' ' * 39}{Style.NORMAL}║", Fore.GREEN)
    fade_in_line(f"║   ▣ {Style.NORMAL}Admin:{Style.BRIGHT} Nguyễn Văn Đạt{' ' * 32}║", Fore.GREEN)
    fade_in_line(f"║   ▣ {Style.NORMAL}YouTube:{Style.BRIGHT} SIÊU CODE{' ' * 28}║", Fore.GREEN)
    fade_in_line(f"║   ▣ {Style.NORMAL}Zalo:{Style.BRIGHT} 0982....{' ' * 28}║", Fore.GREEN)
    fade_in_line(f"║   ▣ {Style.NORMAL}Thời gian:{Style.BRIGHT} {time.strftime('%H:%M:%S')}{' ' * 28}║", Fore.GREEN)
    print(f"{Fore.GREEN}╚═{'═' * 58}═╝{Style.RESET_ALL}")
    
    # Giai đoạn 3: Tiêu đề menu với sóng và gradient
    time.sleep(0.3)
    clear_screen()
    print(f"{Fore.BLUE}╔═{'═' * 58}═╗{Style.RESET_ALL}")
    print(f"{Fore.BLUE}║ {Style.RESET_ALL}{Fore.WHITE}{Style.BRIGHT}{header_text.center(58)}{Style.RESET_ALL} ║")
    print(f"{Fore.BLUE}╚═{'═' * 58}═╝{Style.RESET_ALL}")
    print(f"{Fore.GREEN}╔═{'═' * 58}═╗{Style.RESET_ALL}")
    print(f"║ {Style.BRIGHT}THÔNG TIN HỆ THỐNG{' ' * 39}{Style.NORMAL}║")
    print(f"║   ▣ {Style.NORMAL}Admin:{Style.BRIGHT} Nguyễn Văn Đạt{' ' * 32}║")
    print(f"║   ▣ {Style.NORMAL}YouTube:{Style.BRIGHT} SIÊU CODE{' ' * 28}║")
    print(f"║   ▣ {Style.NORMAL}Zalo:{Style.BRIGHT} 0982....{' ' * 28}║")
    print(f"║   ▣ {Style.NORMAL}Thời gian:{Style.BRIGHT} {time.strftime('%H:%M:%S')}{' ' * 28}║")
    print(f"{Fore.GREEN}╚═{'═' * 58}═╝{Style.RESET_ALL}")
    ...
    # Giai đoạn 4: Danh sách script với viền đôi và ô vuông
    time.sleep(0.2)
    print(f"{Fore.CYAN}╔═{'═' * 58}═╗{Style.RESET_ALL}")
    fade_in_line(f"║ {Style.BRIGHT}CÔNG CỤ HOẠT ĐỘNG{' ' * 40}{Style.NORMAL}║", Fore.CYAN)
    for key, value in SCRIPTS.items():
        status_color = Fore.GREEN if key != "0" else Fore.RED
        fade_in_line(f"║   ▣ {Style.NORMAL}[{key}] {Style.BRIGHT}{value['name']:<36}{Style.NORMAL} [{('BẬT' if key != '0' else 'TẮT')}]", status_color)
    fade_in_line(f"║   ▣ {Style.NORMAL}[0] {Style.BRIGHT}{'Tắt hệ thống':<36}{Style.NORMAL} [TẮT]", Fore.RED)
    print(f"{Fore.CYAN}╚═{'═' * 58}═╝{Style.RESET_ALL}")
    
    # Footer với gradient và hiệu ứng sóng
    time.sleep(0.2)
    symbols = ["✪", "✬", "✯", "✰"]
    footer_text = f" SCODE v1.0 | BY NVĐ {random.choice(symbols)} "
    gradient_sweep(f"╚════{footer_text.center(54)}════╝", [Fore.MAGENTA, Fore.BLUE, Fore.CYAN], 2)
